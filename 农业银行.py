...
# 空字典
bank = {
    72488604: {"username": 'luciano', 'account': 72488604,'acckind':2, 'password': '123456', 'country': 'china', 'province': 'SD',
...
def bank_add(account, username, acckind,password, country, province, street, door):
...
def Useradd():
    ...
    door = input("\t\t请输入您的门牌号")
    add = bank_add(account, username, acckind, password, country, province, street, door)
    if add == 3:
...
def kind(acc):
        if acc in bank:
            if bank[acc]["acckind"]==1:
                return 50000
            else:
                return 20000

def remove():
    acc=int(input("输入要转出的账号"))
